...
/*!
 * @brief This type of structure instance is used to setup the value in configuration when calling the @ref TIM_EnablePWMShift().
 */
typedef struct
{
    uint16_t value; /*!< Counter time value when need pwm output shift. */
} TIM_PWMShift_Type;
 ...
 * @param conf Configurate the counter time value when need pwm output shift. If conf is NULL, disable the shift of the channel. @ref TIM_PWMShift_Type.
 * @return None.
 */
