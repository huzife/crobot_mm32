...
{
    time *= 2;
    LPTIM->TARGET = (bus_clock / 1000) / 2;
    LPTIM->CTRL |= LPTIM_CTRL_LPTEN_MASK;
    while(time --)
    {
        while(!(LPTIM->IF & LPTIM_INT_COUNTER_OVERFLOW));
        LPTIM->IF |= LPTIM_INT_COUNTER_OVERFLOW;
    }
    LPTIM->CTRL &= ~LPTIM_CTRL_LPTEN_MASK;
}
...
{
    LPTIM->TARGET = (bus_clock / 1000000);
    LPTIM->CTRL |= LPTIM_CTRL_LPTEN_MASK;
    while(time --)
    {
        while(!(LPTIM->IF & LPTIM_INT_COUNTER_OVERFLOW));
...
void system_delay_init (void)
{
    ...
    lptim_set.Prescaler = LPTIM_Prescaler_1;
